...
class GameSurface(ScreenHandle):
    ...
    def draw_map(self):
        ...
        min_x, min_y = self.screen_offset()
...
class HelpWindow(ScreenHandle):
    ...
    # FIXME You can add some help information
        self.data.append([" Tab ", "Toggle Minimap"])
        self.data.append(["", ""])
...
class Minimap(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        if self.game_engine.show_minimap:
            if self.game_engine.minimap:
                for i in range(len(self.game_engine.minimap[0])):
                    for j in range(len(self.game_engine.minimap)):
                        self.blit(self.game_engine.minimap[j][i][0], (i*3, j*3))
        
            self.draw_hero()
        super().draw(canvas)
    
    def draw_hero(self):
        self.blit(self.game_engine.hero_marker, (self.game_engine.hero.position[0]*3,
                                      self.game_engine.hero.position[1]*3))
